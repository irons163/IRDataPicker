//
//  IRDataPickerManagerHeaderView.m
//  IRDataPicker
//
...
@interface IRDataPickerManagerHeaderView()
...
@implementation IRDataPickerManagerHeaderView
...
- (void)layoutSubviews {
    ...
    if (self.style == IRDataPickerManagerStyleSheet) {
        ...
        [self setyle1];
    }else if (self.style == IRDataPickerManagerStyle3) {
        [self setyle2];
    }
}
...
- (void)setLanguage:(NSString *)language {
    _language = language;
    NSString *cancelButtonText = [NSBundle IR_localizedStringForKey:@"cancelButtonText" language:self.language];
    [self.cancelButton setTitle:cancelButtonText forState:UIControlStateNormal];
    
    NSString *confirmButtonText = [NSBundle IR_localizedStringForKey:@"confirmButtonText" language:self.language];
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        label.textColor = [UIColor ir_colorWithHexString:@"#848484"];
...
- (UIColor *)confirmButtonTextColor {
    if (!_confirmButtonTextColor) {
        _confirmButtonTextColor = [UIColor ir_colorWithHexString:@"#69BDFF"];
    }
