...
@interface IRPickerView()<IRPickerColumnViewDelegate>
...
@property (nonatomic, strong) NSArray<IRPickerColumnView *> *columnViewList;
...
- (IRPickerColumnView *)createColumnViewAtComponent:(NSUInteger)component refresh:(BOOL)refresh {
    ...
    IRPickerColumnView *view = [self columnViewInComponent:component];
    if (!view) {
        ...
        view = [[IRPickerColumnView alloc]initWithFrame:CGRectMake(component * width, 0, width, kHeight) rowHeight: rowHeight upLineHeight:upLineHeight downLineHeight:downLineHeight];
        view.delegate = self;
...
- (void)setupColumnView {
    ...
    for (int i = 0; i < _numberOfComponents; i++) {
        IRPickerColumnView *view = [self createColumnViewAtComponent:i refresh:false];
...
- (void)setupView {
    ...
    switch (self.type) {
        case IRPickerViewLineTypeline:
        {
            [self setupLineView1];
            return;
        }
        case IRPickerViewLineTypelineSegment:
        {
            [self setupLineView2];
            return;
        }
        case IRPickerViewLineTypelineVertical:
        {
            [self setupLineView3];
            return;
        }
...
- (IRPickerColumnView *)columnViewInComponent:(NSUInteger)component {
    if (!self.columnViewList || self.columnViewList.count == 0) {
        return nil;
    }
    __block IRPickerColumnView *view = nil;
    [self.columnViewList enumerateObjectsUsingBlock:^(IRPickerColumnView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
#pragma mark - IRPickerColumnViewDelegate
...
#pragma mark - Setter
- (void)setDataSource:(id<IRPickerViewDataSource>)dataSource {
